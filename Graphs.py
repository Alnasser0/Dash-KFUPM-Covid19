...
count_cities=len(d.grouped_cumulative_cities.City.unique())
#FIGURES
###########################################################################
###########################################################################
###########################################################################
...
NewMortalities_Bar_City_E = px.bar(d.grouped_daily_cities.sort_values('Date', 
ascending=True).tail(count_cities).sort_values('Mortalities'), x="Mortalities", y="City", 
title="New Deaths", orientation='h')


Mortalities_Bar_City_E = px.bar(d.grouped_cumulative_cities.sort_values('Date', 
ascending=True).tail(count_cities).sort_values('Mortalities'), x="Mortalities", y="City", 
title="All Deaths", orientation='h')


NewMortalities_Bar_E = px.bar(d.grouped_daily, x="Date", y="Mortalities", 
...
Count_Bar_Cases_E = px.bar(d.grouped_daily_melt, x="Date", y="Count", 
...
Count_Line_Cases_E = px.line(d.grouped_cumulative_melt, x="Date", y="Count", 
...
Count_BarArea_Cases_E = px.area(d.grouped_cumulative_melt, x="Date", y="Count", 
...
Mortalities_Scatter_City_E = px.scatter(d.grouped_cumulative_cities.sort_values('Date', 
...
RecoveryRate_Line_E = px.line(d.grouped_cumulative, x="Date", y="Recovary Percent", 
title="Recovary Ratio of Eastern Region Cases")
RecoveryRate_Line_E.update_xaxes(rangeslider_visible=True)


CFR_Line_E = px.line(d.grouped_cumulative[d.grouped_cumulative.Cases >= 100], x="Date", y="CFR Percent", 
title="Cases Fetalitiy Ratio (CFR) of Eastern Region When Cases > 100")
CFR_Line_E.update_xaxes(rangeslider_visible=True)


CFR_Line_City_E = px.line(d.grouped_cumulative_cities[d.grouped_cumulative_cities.Cases >= 10], 
...
df = d.grouped_daily_weekly.copy()
...
df = d.grouped_daily_cities_weekly.copy()
df['Date'] = pd.to_datetime(df['Date'])
df['Date'] = df['Date'].dt.strftime('%m/%d/%Y')
x_min=np.min(df['Cumulative Cases'])+1
x_max=np.max(df['Cumulative Cases'])
...
df = d.grouped_daily_regions_weekly.copy()
df['Date'] = pd.to_datetime(df['Date'])
df['Date'] = df['Date'].dt.strftime('%m/%d/%Y')
x_min=np.min(df['Cumulative Cases'])+1
x_max=np.max(df['Cumulative Cases'])
